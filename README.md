...
1. После запроса бот выдает сообщение "Ошибка при обращении к нейросети". Необходимо проверить подключение и выключить VPN - это может мешать работе бота
2. При попытке запуска контейнеров Docker Compose в терминале появляется ошибка и запуск останавливается. Необходимо перезапустить терминал и ввести эту команду вновь. Иногда возникают проблемы со стороны Docker
